...
/*
	...
	$scope.page_prev = function ()
	{
		...
		$scope.viewer.gotoPage($scope.file_current_page);
	}

	// Click on next.
	$scope.page_next = function ()
	{
		if($scope.file_current_page == $scope.file.Pages.length)
		{
			return false;
		}	
...
*/
	...
	$scope.refresh_file = function ()
	{
		Idbstore.get_by_db_id('Files', $routeParams.id, function (data) {
			...
			$scope.totalPages = data.Pages.length + 1;					
			$scope.$apply();
		});		
	}
