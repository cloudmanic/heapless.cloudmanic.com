...
controllers.main.template = ['$scope', '$rootScope', '$http', '$location', '$fileUploader', 'Broadcaster', 'Rest', 'Idbstore', function($scope, $rootScope, $http, $location, $fileUploader, Broadcaster, Rest, Idbstore)
{
	...
	if(typeof Worker != 'undefined') 
	{
		...
		var stores = site.config.indexeddb.stores.slice(0);
		sync_worker = new Worker('/assets/app/workers/sync.js');

		// Remove non-core stores.
		var indexes = [];
		for(var i in stores)
		{			
			if(! site.config.indexeddb[stores[i]].core)
			{
				indexes.push(stores[i]);
			}
		}

		for(var i in indexes)
		{
			stores.splice(stores.indexOf(indexes[i]), 1);
		}
		...
		sync_worker.addEventListener('message', function(e) {
	  	...
	  	{
				case 'sync-store':
					// Mark this store as returned.
					stores.splice(stores.indexOf(e.data.store), 1);
				
					// Tell everyone a particular store is done.
					Broadcaster.send({ action: 'sync-store', store: e.data.store });
					Broadcaster.send({ action: 'sync-store-' + e.data.store });
				
					// Tell everyone the first sync is done.
					if((! stores.length) && (! site.state.first_synced))
					{
						// Set first sync done.
						...
						// Grab the non-core data.
						sync_worker.postMessage({ action: 'sync-non-core' });	
					} else
					{
						$scope.get_counts();
